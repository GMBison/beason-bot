...
function formatSummary(data) {
  const pricing = getCouponPricing(data.coupon_code);
  return [
    "Please confirm your BEASON CBT order:",
    `Full name: ${data.full_name}`,
    `Email: ${data.email}`,
    `HWID: ${data.hwid}`,
    `Original amount: ${formatAmountNaira(pricing.originalAmountKobo)}`,
    `Discount: ${pricing.discountPercent}%`,
    `Final amount: ${formatAmountNaira(pricing.finalAmountKobo)}`,
    `Coupon code: ${pricing.couponCode || "None"}`,
    "",
    "Reply YES to create your payment link, or NO to go back to the coupon step.",
  ].join("\n");
}
...
async function handleState(chatId, from, text, stateRecord) {
  ...
  if (state === "awaiting_hwid_confirm") {
    ...
    await setBotState(from.id, "awaiting_coupon_code", data);
    await sendTelegramMessage(
      chatId,
      "Do you have a coupon code? Reply NONE if you don't have one."
    );
    return;
  }

  if (state === "awaiting_coupon_code") {
    const normalizedCoupon = String(message || "").trim();
    if (/^none$/i.test(normalizedCoupon)) {
      const nextData = {
        ...data,
        coupon_code: "",
      };
      await setBotState(from.id, "awaiting_order_confirmation", nextData);
      await sendTelegramMessage(chatId, formatSummary(nextData));
      return;
    }

    const pricing = getCouponPricing(normalizedCoupon);
    if (!pricing.couponCode) {
      await sendTelegramMessage(
        chatId,
        "That coupon code is invalid. Reply with the correct coupon code or NONE if you don't have one."
      );
      return;
    }
  ...
  if (state === "awaiting_order_confirmation") {
    if (isNo(message)) {
      await setBotState(from.id, "awaiting_coupon_code", {
        full_name: data.full_name,
        email: data.email,
        hwid: data.hwid,
      });
      await sendTelegramMessage(chatId, "Okay. Reply with your coupon code or NONE.");
      return;
    }
    ...
    const pricing = {
      originalAmountKobo: Number(payment.order.metadata?.original_amount_kobo || payment.order.amount_kobo),
      discountPercent: Number(payment.order.metadata?.discount_percent || 0),
      finalAmountKobo: Number(payment.order.metadata?.final_amount_kobo || payment.order.amount_kobo),
      couponCode: payment.order.metadata?.coupon_code || null,
    };
