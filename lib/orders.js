...
function getOrderPricingSnapshot(order) {
  ...
  return {
    ...
    originalAmountKobo: Number(metadata.original_amount_kobo || order?.amount_kobo || 0),
    finalAmountKobo: Number(metadata.final_amount_kobo || order?.amount_kobo || 0),
  };
}

function doesPendingOrderMatchPricing(order, pricing) {
  const snapshot = getOrderPricingSnapshot(order);
  return (
    String(snapshot.couponCode || "") === String(pricing.couponCode || "") &&
    Number(snapshot.discountPercent) === Number(pricing.discountPercent) &&
    Number(snapshot.originalAmountKobo) === Number(pricing.originalAmountKobo) &&
    Number(snapshot.finalAmountKobo) === Number(pricing.finalAmountKobo) &&
    Number(order?.amount_kobo || 0) === Number(pricing.finalAmountKobo)
  );
}
...
async function createPaymentForOrderInput({ telegramUserId, telegramUsername, fullName, email, hwid, couponCode }) {
  ...
  if (existing && existing.paystack_authorization_url && doesPendingOrderMatchPricing(existing, pricing)) {
    return {
      reused: true,
      order: existing,
  ...
  if (pendingForUser && pendingForUser.paystack_authorization_url && doesPendingOrderMatchPricing(pendingForUser, pricing)) {
    return {
      reused: true,
