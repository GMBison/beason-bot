...
async function createPaymentForOrderInput({ telegramUserId, telegramUsername, fullName, email, hwid }) {
  ...
  const orderInsert = await supabase
    ...
    .insert({
      ...
      hwid: normalizedHwid,
      email,
      amount_kobo: config.license.priceKobo,
  ...
  const payment = await initializeTransaction({
    email,
    amountKobo: config.license.priceKobo,
    reference,
    metadata: {
      order_code: orderCode,
      telegram_user_id: telegramUserId,
